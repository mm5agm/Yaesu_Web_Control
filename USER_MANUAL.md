...
On the panel: pick a USB capture device, set the frame rate from the rates the stick advertises, Fit/Fill, Fullscreen, Pop out / **Reattach**, or Close.
...
3. Frame rate and image quality (**Low / Medium / Max** = 40 / 65 / 85; default **Medium**) are chosen on the same card. The FPS list is read from the capture device (for example **30 / 60** on a typical HDMI stick). If the driver does not report rates, **15 / 30 / 40 / 60** is shown. The selected value is a **target** — USB bandwidth, JPEG encode, and host CPU can still deliver less. Higher rates and **Max** quality use more CPU — prefer a lower FPS and **Low** or **Medium** on a Raspberry Pi. **Max** is crisper on digits when you enlarge the image; the 85 cap is intentional.
4. Other controls:
