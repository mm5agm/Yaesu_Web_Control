...
**Passband** — tick the **Passband** box (beside the span buttons) to draw the receiver's IF passband as a shaded band on the trace, with the amber dial marker inside it. Its width and position follow the radio's IF Width and IF Shift, so it shows exactly which slice of the band you are listening to, and it moves as you change either control. On the FTdx101 the marker sits at the station's true frequency — see the note on where signals are drawn in §6.3. In **DATA-L** and **DATA-U**, YWC assumes the radio's **DATA SHIFT (SSB)** menu (FUNC → RADIO SETTING → MODE PSK/DATA) is at its default of 1500 Hz, because it cannot read that menu yet; if you have changed it, signals in DATA modes are drawn off by the difference. The setting is remembered per VFO across browser reloads.
