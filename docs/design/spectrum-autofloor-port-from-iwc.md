...
- **§2.2 persistence — no backend schema change.** Range is a pure *display* knob, so
  ...
  own it; `wireDspSliders()` in `Index.cshtml` inits the slider from the panel.
- **§2.3 waterfall — already fixed-window.** YWC's `_dbToColor` was already keyed to a
  fixed `(db+120)/120` window, not `_dbMin/_dbMax`, so it needed no change. With the
  trace now auto-ranging (a uniform dB shift cancels out), the **Gain** slider no
  longer moves the trace — so it was relabelled **"waterfall brightness"** (it still
  drives the worker's pre-dB gain, which now reads through purely as waterfall colour).
...
- **§2.5 re-seed triggers** — `snapAutoFloor()` (sets `_autoFloorDb = null`) fires on a
  span/bin-count change (`update()`'s `retuned` flag), a large VFO jump
  (`AUTOFLOOR_SNAP_JUMP_HZ` in `setVfoFrequency`), and a Gain change (from
  `wireDspSliders`).

Not yet done: manual browser verification (both receivers streaming, Range sweep,
band/Gain re-settle, waterfall no-strobe) per §4.5 — awaiting a bench session.
