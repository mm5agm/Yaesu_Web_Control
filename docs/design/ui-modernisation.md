...
**Status:** the theming half is now implemented for YWC — see
"What was actually built" at the foot of this document. The layout half
(direction C) remains exploration, and is being taken up in Icom Web
Control first.
...
## What was actually built (2026-09-13, `feature/themes`)

Themes only. No layout change, no control moved, nothing added or removed
from any page.

**In `core/`, shared with IWC:**

| file | what it is |
|---|---|
| `core/css/theme-tokens.css` | every value a theme may change. `:root` holds the Classic values — i.e. what both apps already ship — and `[data-theme="instrument"]` overrides them |
| `core/css/theme-instrument.css` | the Bootstrap layer: cards, buttons, forms, tables, modals, focus. Both apps are Bootstrap 5.3.0, so this is written once |
| `core/js/theme/theme-boot.js` | classic render-blocking script; applies the per-device override before first paint |
| `core/js/theme/theme.js` | ES-module API over it, plus `themeToken` / `themePalette` / `onThemeChange` for canvas code |

**In YWC:**

| file | what it is |
|---|---|
| `wwwroot/css/theme-instrument-local.css` | the components named after YWC's own markup — band radiogroup, VFO column, calibration pages, keypad and memory dialogs |
| `Yaesu_Web_Control.csproj` | `CopySharedCoreCss`, mirroring `CopySharedCoreJs` |
| `Models/ApplicationSettings.cs` | `Theme`, defaulting to `"Classic"` |
| `Pages/Shared/_Layout.cshtml` | renders `data-theme` on `<html>`, links the sheets, loads the boot script |
| `Pages/Settings.cshtml` | an Appearance section with live preview |

### The three decisions worth knowing

**1. Classic is unreachable, not merely unchanged.** Every rule in both
Instrument sheets is prefixed `[data-theme="instrument"]`. Without that
attribute on `<html>`, not one selector matches. This is a stronger claim
than "we tested it and it looked the same", and it is the reason the
change is safe to review quickly.

The one exception is deliberate: `site.css`'s `.frequency-display:focus`
rule now reads its three values from tokens whose `:root` defaults are
exactly the literals it used to contain. Tokenising an existing rule where
it lives is better than writing a scoped override beside it — but only
where the default is provably identical.

**2. The theme rides on Bootstrap's own dark mode.** Bootstrap 5.3 has
`data-bs-theme="dark"`, and some of what it switches cannot be reached
from a CSS variable at all — the chevron on a `<select>` and the tick on a
`.form-check` are data-URI background images picked by selector. So a dark
theme sets both attributes: Bootstrap's dark substrate, our palette over
it. Without this, selects and checkboxes keep dark glyphs on dark controls.

**3. Presentational accessibility is the whole exposure.** The 396
`aria-*`, 42 `role=` and 83 `data-a11y-key` attributes measured above live
in markup, and a stylesheet cannot reach any of them — a theme inherits
all of that for free. What a theme *can* silently destroy is the part made
of CSS: contrast, the focus ring, and hit-target size. Four tokens carry
it, each with its WCAG clause written next to it in
`core/css/theme-tokens.css`.

Two of the mockups' colours failed when the ratios were actually computed
and ship corrected: the caption grey `#6d8296` was 4.27:1 at the 10–11px
it is used at (1.4.3 wants 4.5), and the alarm red `#e2453f` was 4.17:1 —
which is the colour that says *you are transmitting out of band*. A third
was found by computation rather than inspection: no plausible dark border
reaches the 3:1 boundary for a UI component (1.4.11), so
`--rwc-border-control` reads lighter than a designer would pick by eye and
carries a comment saying not to darken it back.

### Known not covered

The meters, the SDR spectrum, the waterfall and the filter scope are
painted by JavaScript into a `<canvas>`. CSS reaches none of it, so under
Instrument they keep their present light faces on a dark page. That is a
real rough edge in the middle of the main screen.

Fixing it means the drawing code reading its palette from the tokens
instead of hard-coding it. `core/js/theme/theme.js` already provides the
seam (`themeToken`, `themePalette`, `onThemeChange`) and nothing yet calls
it. It is a separate change on purpose: it touches the gauge and spectrum
pipelines, which are the parts of the frontend with the strictest
architecture rules and the least test cover, and it should not ride along
inside a stylesheet change.

### What this means for direction C

The layout work is being started in IWC, and the split turned out to be
cleaner than expected. What is shareable is the engine — the panel model,
the grid/dock geometry, layout presets, persistence — because none of it
knows what a panel contains. What is not shareable is each app's panel
catalogue, which is built from its own `RadioCapabilities`, and each app's
markup.

That matters here because the same seam does both jobs. A theme and a
layout are the same kind of change from the operator's point of view: the
controls are identical and identically labelled, and only their
presentation differs. Building the theme first proves the cheaper half of
that claim before the expensive half is attempted.
