/**
 * Dock workspace Remote Audio UI (/Dock only).
...
function initRemoteAudioTooltips() {
  ...
  if (!bar || bar.dataset.ywcAudioUi !== 'dock') return false;
...
/** Wire Start/Stop/mute/meters for the Dock remote-audio panel. */
function bindRemoteAudioDockControls() {
...
/**
 * Dock workspace Remote Audio panel (/Dock) — full inline controls, native styling.
 */
export async function initRemoteAudioDock() {
  const bar = document.getElementById('remoteAudioBar');
  if (!bar || bar.dataset.ywcAudioUi !== 'dock') return;
