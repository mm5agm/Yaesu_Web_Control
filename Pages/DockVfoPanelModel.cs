...
{
    /// <summary>
    /// Per-VFO bindings for the shared Dock <c>_Vfo</c> partial (A or B).
    /// </summary>
    public sealed class DockVfoPanelModel
    {
        public DockModel Page { get; }
        public char Vfo { get; }

        private DockVfoPanelModel(DockModel page, char vfo)
        {
            Page = page;
            Vfo = char.ToUpperInvariant(vfo);
        }

        public static DockVfoPanelModel A(DockModel page) => new(page, 'A');
        public static DockVfoPanelModel B(DockModel page) => new(page, 'B');
        ...
        public DockViewState View => Page.View;
