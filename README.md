...
- **An RTTY tuning scope.** The **RTTY Tune** button opens the crossed-ellipse display that used to sit beside every RTTY terminal unit. The receive audio goes through a mark filter and a space filter, one drives the trace across and the other up, and you tune for a clean cross. The arms lean and open up when you are off tune, and one goes missing when the shift or polarity is wrong. It follows the mode: RTTY-L, measured here, puts space above mark; RTTY-U puts it below, which I have not yet measured. It uses the same USB audio as the CW Reader and transmits nothing. [§22](USER_MANUAL.md#22-rtty-tuner).
...
|---|---|---|
| In **DATA-L** and **DATA-U** the IF Width dropdown showed the SSB widths, but the radio uses its narrower CW/PSK set in DATA, so what you picked was not what you got: "2.4 kHz" gave 1.4 kHz, and "1.8 kHz" gave 450 Hz. The dropdown now shows the widths the radio actually uses in DATA (measured), and the Filter Function Display and the spectrum's shaded passband draw them where the radio puts them. For FT8 and other wide data modes, pick **3.0 kHz** or wider ([§5.8](USER_MANUAL.md#58-if-width-audio-filter-if-shift-and-af-gain)). | [#172](https://github.com/mm5agm/Yaesu_Web_Control/discussions/172) | Not yet in a build |
| In **DATA-L** and **DATA-U**, signals on the spectrum were only drawn in the right place if the radio's **DATA SHIFT (SSB)** menu was at its default of 1500 Hz. The radio centres its DATA filter on that menu, so anyone who had changed it saw every signal off by the difference. On the FTdx101, YWC now reads DATA SHIFT from the radio when a VFO goes into a DATA mode, and the trace and passband follow it (measured at 1000 and 1500 Hz) ([§5.4](USER_MANUAL.md#54-spectrum-display)). | [#172](https://github.com/mm5agm/Yaesu_Web_Control/discussions/172) | Not yet in a build |
| On the FTdx101, signals on the spectrum were drawn in the wrong place in **LSB**, **DATA-L** and **CW-L**, and a little off in USB, DATA-U and AM: in LSB a station was drawn about 2.6 kHz right of the dial, on the wrong side of the shaded passband, and in DATA-L a signal you were copying was drawn on the dial instead of about 1.5 kHz below it. Where the radio moves its filter is now worked out for each sideband (re-measured), so a station is drawn inside the shaded passband where you hear it. | [#172](https://github.com/mm5agm/Yaesu_Web_Control/discussions/172) | v2.5.2-pre9 |
| In the radio's own **RTTY-L** and **RTTY-U**, the spectrum's shaded passband was drawn as if they were plain LSB and USB, 1.2-1.8 kHz away from the signal the radio was actually listening to. It is now drawn over the two tones, where the radio's RTTY filter really is (measured), and the filter display under the S-meter centres RTTY on 2210 Hz instead of 1500 ([§5.4](USER_MANUAL.md#54-spectrum-display)). | [#178](https://github.com/mm5agm/Yaesu_Web_Control/issues/178) | v2.5.2-pre9 |
| In **DATA-L**, click-to-tune always put a RTTY signal's tones on 2125 and 2295 Hz, however your RTTY software was set. With the 1415 Hz mark many operators use, every click landed 710 Hz off. It now uses the **Mark**, **Shift** and **Rev** set in the RTTY Tuner, so type your software's mark in there once ([§5.4](USER_MANUAL.md#54-spectrum-display), [§22.2](USER_MANUAL.md#222-mark-shift-and-rev)). | [#169](https://github.com/mm5agm/Yaesu_Web_Control/discussions/169) | v2.5.2-pre9 |
| Clicking a RTTY signal on the spectrum tuned to the wrong place in both RTTY modes people use. In the radio's own **RTTY-L** YWC put the dial 2.2 kHz above the signal, as if the dial were the suppressed carrier, but on the FTdx101 it sits on the signal's mark tone (measured), so every click landed outside the 500 Hz RTTY filter and you heard nothing. In **DATA-L**, the usual mode for AFSK RTTY, it did the opposite and tuned straight onto the signal, leaving the tones near 0 Hz audio where no software can decode them. Both now land where the mode needs them; DATA-U (FT8 and the like) still tunes straight onto the click ([§5.4](USER_MANUAL.md#54-spectrum-display)). | [#169](https://github.com/mm5agm/Yaesu_Web_Control/discussions/169) | v2.5.2-pre8 |
