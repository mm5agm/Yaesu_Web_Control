...
|---|---|---|
| With the automatic mode change switched off, clicking a RTTY signal on the spectrum in **DATA-L** tuned the dial straight onto it, which left the tones near 0 Hz audio where nothing can decode them, so every click had to be re-tuned by hand. The RTTY tune offset only covered the radio's own RTTY-L and RTTY-U, and AFSK RTTY in DATA-L is how many contesters run it. DATA-L now gets the same offset as RTTY-L; DATA-U (FT8 and the like) still tunes straight onto the click ([§5.4](USER_MANUAL.md#54-spectrum-display)). | [#169](https://github.com/mm5agm/Yaesu_Web_Control/discussions/169) | Not yet in a build |
